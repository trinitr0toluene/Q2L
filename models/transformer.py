...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=0,
        ...
        self.pro_m_t = nn.Linear(49, 28)
        # self.pro_t_m = nn.Linear(28,49)
        # self.att = nn.Softmax(dim=0)
        self.att = nn.Sigmoid()
    ...
    def forward(self, src, query_embed, pos_embed, mask=None):
        ...
        #     tgt = query_embed.permute(1, 0, 2)
        #     query_embed1 = self.pro_m_t(memory.permute(2,1,0))
        #     query_embed1 = self.att(query_embed1.permute(2,1,0))
        # else:
        query_embed1 = query_embed.unsqueeze(1).repeat(1, bs, 1)
        tgt1 = self.pro_m_t(memory.permute(2, 1, 0))
        tgt = self.att(tgt1.permute(2,1,0))
        # tgt = torch.zeros_like(query_embed1)
        ...
        for layer in self.layers:
            ...
            if self.dataname == 'intentonomy' and curr_layer == 0:
                for i in range(len(CLASS_15)):
                    ...
                    # output_inter_trans[i] = inter
                # inter_qurpos = self.proj_m_t1(memory.permute(2, 1, 0))
                # query_pos = self.att(inter_qurpos.permute(2,1,0))
                query_pos = output_inter_trans
                inter_tgt = self.proj_m_t1(memory.permute(2, 1, 0))
                output_inter = self.att(inter_tgt.permute(2,1,0))
                # output_inter = torch.zeros_like(output_inter_trans)
                ...
                for i in range(len(CLASS_9)):
                    ...
                    if i == 0:
                        ...
                        output_inter_trans = torch.cat((output_inter_trans, inter.unsqueeze(0)), 0)
                # inter_qurpos = self.proj_m_t2(memory.permute(2, 1, 0))
                # query_pos = self.att(inter_qurpos.permute(2, 1, 0))
                query_pos = output_inter_trans
...
def build_transformer(args):
    return Transformer(
        ...
        # use_label_features = args.use_label_features,
