...
class Qeruy2Label(nn.Module):
    def __init__(self, backbone, transfomer, num_class, dataname):
        ...
        #     self.query_embed = nn.Linear(768, hidden_dim)
        #     # self.query_embed2 = nn.Embedding(num_class,hidden_dim)
        # else:
        ...
        if self.dataname == 'intentonomy':
            ...
            self.fc_coarse = GroupWiseLinear(25, hidden_dim, bias=True)
    ...
    def forward(self, input):
        ...
        # if self.use_label_features:
        #     query_input = self.query_embed(label_features)
        #     # query_input2 = self.query_embed2.weight
        # else:
        ...
        # image = self.avgpool(src)
        # image = image.flatten(2).permute(0, 2, 1)
        # out.append(self.fc_num_class(image))
        # out = self.fc(hs[-1])
        return out
